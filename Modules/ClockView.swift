...
struct ClockFaceCanvas: View {
    ...
    func drawFace(context: GraphicsContext, size: CGSize) {
        ...
        context.stroke(circle_path,
                       with: .linearGradient(Gradient(colors: [.primary, .primary]),
                                             startPoint: .zero,
                                             endPoint: CGPoint(x: size.width, y: size.height)),
                       lineWidth: size.width * 0.05)
        
        // Background
        let gradient = Gradient(stops: [.init(color: .clear, location: 0),
    ...
    func drawTicks(context: GraphicsContext, size: CGSize) {
        ...
        for tick in 0...59 {
            ...
            context.fill(tick % 5 == 0 ? thick : thin, with: .color(.primary))
        }
    }
...
struct ClockHandsCanvas: View {
    ...
    func drawHands(context: GraphicsContext, size: CGSize, date: Date) {
        ...
        context.fill(dot1_p, with: .color(.primary))
